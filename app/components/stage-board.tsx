...
import { SortableLayer } from "~/components/sortable-layer"
...
export function StageBoard({ stage }: StageBoardProps) {
  const { layers, projects, moveProject, updateLayerOrder } = useContentPlannerContext()
  ...
  function handleDragEnd(event: any) {
    ...
    if (active.data.current?.type === "layer" && over.data.current?.type === "layer") {
      if (active.id !== over.id) {
        const oldIndex = stageLayers.findIndex((layer) => layer.id === active.id)
    ...
    if (active.data.current?.type === "project" && over.data.current?.type === "layer") {
  ...
  return (
    ...
    >
      <div className="flex gap-4 overflow-x-auto pb-4">
        <SortableContext items={stageLayers.map((layer) => layer.id)} strategy={horizontalListSortingStrategy}>
          {stageLayers.map((layer) => {
            const layerProjects = projects.filter((project) => project.layerId === layer.id)

            return <SortableLayer key={layer.id} layer={layer} projects={layerProjects} />
          })}
        </SortableContext>
      </div>

      <DragOverlay>{activeId && activeProject ? <ProjectCard project={activeProject} /> : null}</DragOverlay>
    </DndContext>
  )
}
